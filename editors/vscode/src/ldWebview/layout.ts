...
import { BlockOutput, CoilOutput, ContactElement, LdProgram } from './model';
...
export const CELL: LayoutConstants = {
  contactW: 72,
  contactH: 28,
  blockW: 84,
  blockH: 60,
...
export type ElementKind = 'contact' | 'coil' | 'block';
...
export type WireKind = 'rail' | 'series' | 'collector' | 'tee';

/**
 * What a wire carries, for power-flow coloring:
 * - `source` — supply side (left rail, collector): live whenever flow exists.
 * - `contact` — segment right of `after`-th contact of `branch`: live when
 *   `contact_energized[branch][after]`.
 * - `branch` — trailing segment of `branch`: live when the branch conducts.
 * - `rung` — tee→output feed: live on `rung_result`.
 * - `output` — output→right-rail: live on `output_energized[index]`.
 * - `return` — right rail: live when any output is energized.
 */
export type WireCarrier =
  | { type: 'source' }
  | { type: 'contact'; branch: number; after: number }
...
function contactLabel(contact: ContactElement): string {
  return `${contact.negated ? '|/|' : '| |'} ${contact.name}`;
}

function coilLabel(coil: CoilOutput): string {
  const symbol = coil.variant === 'set' ? '(S)' : coil.variant === 'reset' ? '(R)' : '( )';
  return `${symbol} ${coil.name}`;
}

function outputSize(output: LdProgram['rungs'][number]['outputs'][number], cell: LayoutConstants) {
...
export function layout(program: LdProgram, cell: LayoutConstants = CELL): LayoutGeometry {
  ...
  const contactX = (column: number): number =>
    leftRailX + cell.railPadX + column * (cell.contactW + cell.gapX);
  const outputsX = contactX(maxContacts) + cell.gapX;
  const maxOutputW = Math.max(
    cell.coilW,
    ...program.rungs.flatMap((r) => r.outputs.map((o) => outputSize(o, cell).width)),
  );
  ...
  for (let rungIndex = 0; rungIndex < program.rungs.length; rungIndex += 1) {
    ...
    const branchTop = (branch: number): number => y + branch * (rowH + cell.branchGapY);
    const branchMid = (branch: number): number => branchTop(branch) + rowH / 2;

    const outputsTotalH = rung.outputs.reduce(
      (sum, output, index) =>
        sum + outputSize(output, cell).height + (index > 0 ? cell.branchGapY : 0),
      0,
    );
    ...
    const outputY = (index: number): number => {
      let oy = outputsTop;
    ...
    const outputMid = (index: number): number => {
    ...
    for (let o = 0; o < rung.outputs.length; o += 1) {
      ...
      elements.push({
        ...
        width: size.width,
        height: size.height,
        label: output.kind === 'block' ? (output as BlockOutput).fb_type : coilLabel(output as CoilOutput),
        sublabel: output.kind === 'block' ? (output as BlockOutput).instance : undefined,
      });
...
export type InsertionLocation =
  | { kind: 'series'; rung: number; branch: number; index: number }
  | { kind: 'parallel'; rung: number }
  | { kind: 'output'; rung: number; index: number }
  | { kind: 'newRung' }
  | { kind: 'element'; id: string; rung: number; branch: number; index: number };
