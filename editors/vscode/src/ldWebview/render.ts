...
import { LayoutGeometry, PositionedElement } from './layout';
...
function wireEnergized(
  wire: import('./layout').WireSegment,
  geometry: LayoutGeometry,
  flow: PowerFlow | undefined,
): boolean {
  if (wire.kind === 'rail') {
    ...
    const band = geometry.elements.filter(
      (e) => e.y <= midY && e.y + e.height >= wire.y1,
    );
...
function isElementEnergized(element: PositionedElement, flow: PowerFlow | undefined): boolean {
